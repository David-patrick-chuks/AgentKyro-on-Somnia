...
async function testGeminiParser() {
  try {
    ...
    const prompt = `
You are AgentKyro's AI intent parser. Analyze the user message and extract the intent with high accuracy.
...
1. "transfer", "send", "pay" - Send tokens to someone
   - Extract: amount, token (default STT), recipient
   - Example: "Send 50 STT to Alice" → action: "transfer", amount: "50", token: "STT", recipient: "Alice"

2. "balance", "check" - Check wallet balance
   - Extract: token (default STT)
   - Example: "Check my balance" → action: "balance", amount: "0", token: "STT", recipient: ""
...
5. "analytics", "show_analytics", "dashboard" - Show analytics
   ...
   - Example: "Show my analytics" → action: "analytics", amount: "0", token: "STT", recipient: ""

6. "security_check" - Check security
   - Example: "Check my security" → action: "security_check", amount: "0", token: "STT", recipient: ""

7. "sharing" - Share transactions
   - Example: "Share my transaction" → action: "sharing", amount: "0", token: "STT", recipient: ""

8. "notifications" - Manage notifications
   - Example: "Show notifications" → action: "notifications", amount: "0", token: "STT", recipient: ""

9. "transaction_history" - View transaction history
   - Example: "Show transaction history" → action: "transaction_history", amount: "0", token: "STT", recipient: ""

10. "advanced_transactions" - Advanced transaction features
    - Example: "Schedule transaction" → action: "advanced_transactions", amount: "0", token: "STT", recipient: ""

RULES:
- Always set confidence between 0.0 and 1.0
- For transactions: amount must be positive number, token must be "STT" or "ETH"
- For non-transactions: amount = "0", token = "STT", recipient = ""
